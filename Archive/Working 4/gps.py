...
def init():
    uart = UART(2, baudrate=9600, bits=8, parity=None, rx=RX_PIN, tx=TX_PIN, stop=1)
    uart.init()
...
def acquire_data(uart, print_time=False):
    ...
    if collect_flag == False:
        ...
        #buf = uart.readline().decode('ascii', errors='ignore')
...
def main():
    ...
    while True:
        try:
            ...
            time.sleep_ms(100)
            cur_time = time.time()
            if (((cur_time - start_time) % 5) < 0.1) and ((cur_time - start_time) != last_print_time):
